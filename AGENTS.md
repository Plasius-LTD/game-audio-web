...
- Use `/Users/philliphounslow/plasius/schema` (`@plasius/game-audio-web`) as the baseline template when creating new `@plasius/*` packages.
...
### Local Notes
This repo is `@plasius/game-audio-web`, browser runtime lifecycle contracts for lazy Web Audio initialization, playback planning, and graceful fallback.
...
### Repo conventions
- Source lives in `src/`; tests live in `tests/`; package ADRs live in `docs/adrs/`.
